...
## 1.00
* Changed versioning from caldate to semantic. No other differences from 2026.03.24a.
...
* Added a medium glow level similar to the old high. Increased high glow significantly.
...
* Changed icon behavior. Instead of displaying a `?` if an icon fails to load, the card will be shown without the spacing for an icon. You can utilize this feature if you want only some cards to not have an icon. The readme explains how to accomplish this task.
